    ...
    return (
      <div className="flex flex-col items-center">
        <div className="mb-3 sm:mb-4 p-2 sm:p-3 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-lg text-center w-full">
          ...
          <div className="font-mono text-xs sm:text-sm">{address?.substring(0, 6)}...{address?.substring(address.length - 4)}</div>
        </div>
        <button 
          onClick={() => disconnect()} 
    ...
    <div className="flex flex-col items-center w-full">
      ...
      {isOpen && (
        <div className="mt-3 sm:mt-4 p-3 sm:p-4 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg bg-white dark:bg-gray-800 w-full">
          <h3 className="text-center font-medium mb-2 sm:mb-3 text-gray-700 dark:text-gray-300 text-sm sm:text-base">选择连接方式</h3>
          {connectors.map((connector) => (
            ...
            >
              {connector.name === 'MetaMask' && (
                <svg className="w-5 h-5 sm:w-6 sm:h-6 mr-2" viewBox="0 0 35 33" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              {isLoading && pendingConnector?.uid === connector.uid && (
                <svg className="animate-spin ml-2 h-3 w-3 sm:h-4 sm:w-4 text-indigo-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          {error && <div className="text-red-500 mt-2 text-xs sm:text-sm">{error.message}</div>}
        </div>
      )}
    </div>
  )
